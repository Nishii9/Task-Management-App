...
exports.postTask = async (req, res) => {
  try {
    ...
    const task = await Task.create({ user: req.user.id, description, priority, dueDate });
...
exports.putTask = async (req, res) => {
  try {
    ...
    task = await Task.findByIdAndUpdate(req.params.taskId, { description, priority, dueDate }, { new: true });
...
exports.updateTaskStatus = async (req, res) => {
  try {
    ...
    res.status(200).json({ task, status: true, msg: "Task status updated successfully" });
  } catch (err) {
    console.error(err);
    return res.status(500).json({ status: false, msg: "Internal Server Error" });
  }
